...
That validation exposed twelve real defects — all of them general, none of them
specific to that codebase. Each became a generic fixture, then a fix, then a
re-validation, in the order §24 prescribes. The conformance suite is 69/69.
...
69 conformance fixtures, all passing: 0 false negatives, 0 false positives, 0
...
Nineteen fixtures beyond the specification's 50 cover cases found while building
...
global object qualifiers, member chains, a keyword read as a qualifier, choosing between duplicate copies of a type, the arguments of RaiseEvent and With, and resolution for files belonging to no project.

### Regressions — **none**

`npm test`: 164 files, 2970 tests passing, 0 failures. No existing language is
affected (§19).
...
recovered, and three rounds of fixes raised resolution from 28.5% to **72.3%**.
The measure that matters more is **internal call-graph coverage — 86.6%**, or
**96.8%** once the calls VB6 itself cannot reach unqualified are excluded.
